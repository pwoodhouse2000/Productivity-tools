#
# COPY THIS ENTIRE, CLEANED-UP FILE
#
...
def get_notion_projects():
    ...
    response = requests.post(f"https://api.notion.com/v1/databases/{database_id}/query", headers=headers, json={})
    response.raise_for_status()
...
def get_notion_categories():
    ...
    try:
        ...
        headers = get_notion_headers()
        response = requests.post(f"https://api.notion.com/v1/databases/{categories_db_id}/query", headers=headers, json={})
        response.raise_for_status()
        categories = {}
...
def create_or_update_notion_project(project_data, existing_projects, all_todoist_projects, category_map):
    ...
    if page_id:
        response = requests.patch(f"https://api.notion.com/v1/pages/{page_id}", headers=headers, json={"properties": properties})
    else:
        response = requests.post("https://api.notion.com/v1/pages", headers=headers, json={"parent": {"database_id": database_id}, "properties": properties})
    response.raise_for_status()
    return response.json()
...
def create_or_update_notion_task(task_data, project_map, label_map):
    ...
    properties = {
        ...
        "Done": {"checkbox": task_data.get("is_completed", False)}
    }
    ...
    if not page_id:
        ...
        task_ref.set({'todoist_id': str(task_data["id"]), 'notion_id': notion_id, 'last_synced': datetime.now()})
    return response.json()
...
def create_or_update_todoist_task(notion_task, project_map_reverse, existing_labels):
    ...
    if "Type" in notion_task["properties"] and notion_task["properties"]["Type"].get("select"):
        ...
        task_data["labels"] = [type_name] # Corrected from label_id
    if todoist_id:
        response = requests.post(f"https://api.todoist.com/rest/v2/tasks/{todoist_id}", headers=headers, json=task_data)
        response.raise_for_status()
        if is_done:
            requests.post(f"https://api.todoist.com/rest/v2/tasks/{todoist_id}/close", headers=headers).raise_for_status()
    else:
        response = requests.post("https://api.todoist.com/rest/v2/tasks", headers=headers, json=task_data)
        response.raise_for_status()
        new_task = response.json()
        db.collection('task_mappings').document(str(new_task["id"])).set({'todoist_id': str(new_task["id"]), 'notion_id': notion_id, 'last_synced': datetime.now()})
        if is_done:
            requests.post(f"https://api.todoist.com/rest/v2/tasks/{new_task['id']}/close", headers=headers).raise_for_status()
    return response.json()

def sync_all():
    """Main sync function"""
    print("✅ Sync function started.")
    results = {"projects": {"created": 0, "updated": 0, "errors": []}, "tasks": {"created": 0, "updated": 0, "errors": []}, "timestamp": datetime.now().isoformat()}
    try:
        # Initial data fetch
        ...
        notion_projects_refreshed = get_notion_projects() # Refresh to get newly created projects
        for project in notion_projects_refreshed:
            if not project["properties"].get("Source", {}).get("select"):
                try:
        ...
        for task in todoist_tasks:
            try:
                create_or_update_notion_task(task, project_map, label_map)
            except Exception as e:
...
def sync_projects(request):
    ...
    try:
        results = sync_all()
        message_parts = []
        if results["projects"]["created"] > 0: message_parts.append(f"Projects created: {results['projects']['created']}")
        if results["projects"]["updated"] > 0: message_parts.append(f"Projects updated: {results['projects']['updated']}")
        if results["tasks"]["created"] > 0: message_parts.append(f"Tasks created: {results['tasks']['created']}")
        if results["tasks"]["updated"] > 0: message_parts.append(f"Tasks updated: {results['tasks']['updated']}")
        ...
        if total_errors > 0: message += f" Errors: {total_errors}."
