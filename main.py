...
def create_notion_project(project_name, project_id):
        ...
        "properties": {
            ...
            "Description": {
                "rich_text": [
                    {
                        "text": {
                            "content": f"Synced from Todoist (ID: {project_id})"
                        }
                    }
                ]
            },
    ...
    if response.status_code != 200:
        raise Exception(f"Failed to create Notion project: {response.text}")
...
def sync_todoist_to_notion():
    ...
    # Extract existing project names from Notion
    existing_names = set()
    ...
    for project in todoist_projects:
        ...
        if project_name in existing_names:
            ...
            try:
                create_notion_project(project_name, project["id"])
                results["created"] += 1
...
def sync_projects(request):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Sync complete! Created {results['created']} new projects in Notion.",
            "details": results
        }
